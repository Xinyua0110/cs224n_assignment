...
#在word2vec以及绝大部分情况中矩阵都是row vector的形式
    ...
    #soft Max（）：Compute the softmax function for each row of the input x.

    v_c ,o , U=centerWordVec, outsideWordIdx , outsideVectors
    y_predicted= softmax(np.dot(U,v_c))
    loss = -log(y_predicted[o])
    y_predicted[o] -=1 # y_predicted-y;y is a one-hot vector for y[o]=1
    gradCenterVec = np.dot(U.T,(y_predicted))
    gradOutsideVecs =np.dot(y_predicted[:, np.newaxis],v_c[:, np.newaxis].T)
    ...
    '''
    P = softmax(np.dot(centerWordVec,outsideVectors.T))
    loss = -np.log(P[outsideWordIdx])
    P[outsideWordIdx] -= 1
    centerWordVec, outsideVectors = symbols('centerWordVec, outsideVectors',real=True)
    gradCenterVec = diff(loss, centerWordVec)  # 偏导数
    gradOutsideVecs = diff(loss, outsideVectors)
...
def negSamplingLossAndGradient(
    ...
    v_c ,o , U=centerWordVec, outsideWordIdx , outsideVectors
    y_predicted = sigmoid(np.dot(U,v_c))
    loss = -log(y_predicted[o])
    y_predicted[o] -=1 
    gradOutsideVecs,gradCenterVec=np.zeros(U.shape),np.zeros(v_c.shape)

    gradCenterVec = y_predicted[o]*U[o]
    gradOutsideVecs[o] = v_c*y_predicted[o]


    for k in range(len(indices)):
        if indices[k]!=o:
            neg_idx = indices[k]
            neg_sigmoid= sigmoid(-np.dot(U[neg_idx],v_c))
            loss -= log(neg_sigmoid)
            gradCenterVec += (1-neg_sigmoid)*U[neg_idx]
            gradOutsideVecs[neg_idx] -= (neg_sigmoid-1)*v_c
    ...
    gradCenterVec += U[o]*y_predicted[o]
    gradOutsideVecs[o] += v_c*y_predicted[o]
    ...
    for k in range(len(indices)):
        if indices[k]!=o:
            neg_idx=indices[k]
            neg_sigmoid = sigmoid(-np.dot(U[neg_idx],v_c))
            loss -= np.log(neg_sigmoid)
            gradCenterVec += np.multiply(U[neg_idx],1-neg_sigmoid)
            gradOutsideVecs[neg_idx] += np.multiply(1-neg_sigmoid,v_c)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    outsideVectors,
    dataset)
    loss, gradCenterVec, gradOutsideVecs
    ...
    m,U,V,idx_c,J = windowSize,outsideVectors,centerWordVectors,word2Ind[currentCenterWord],word2vecLossAndGradient
    v_c = centerWordVectors[idx_c]
    for i in outsideWords:
        out_idx=word2Ind[i]
        loss_i,gradC_i,gradO_i = J(v_c,out_idx,U,dataset)
        loss += loss_i
        gradCenterVecs[idx_c] += gradC_i
        gradOutsideVectors += gradO_i
